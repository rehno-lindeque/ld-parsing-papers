...
A small pseudo instruction set is developed with the capability to resolve grammar rules containing cycles in their lookahead sets.
...
The identifying rule tokens are placed into a temporary buffer---the \emph{rules buffer}---that grows in a stack-like manner, but also supports constant-time random-access operations.
...
By separating these two aspects into distinct passes, the recognition pass is allowed the convenience of identifying the right hand sides of grammar rules in a more flexible order. 
However, the translation pass requires the rules buffer it is given to be correctly ordered when building the parse tree.
For this purpose, the mechanism used to reorder rules relies on placeholder tokens in the rules buffer representing unresolved grammar rules.
...
However, a some kind of history is needed to retain knowledge of all of the lookahead that has been recognized.

A $goto$ operation can be described as type of `trampoline'.
Depending on the last value assigned to $lasttarget$ by a $switch$ operation, $goto$ will direct the parser to jump to a new state (without changing the value of $lasttarget$). 
This variable $lasttarget$ is representative of all the relevant branches that that the parser needed to follow in order to recognize the needed lookahead up to this point.
...
While it is tempting to classify an LD parser performance according to the number of operations per input token, we should also take into account the class of the grammar that the parser represents.
For simplicity only the recognition pass is considered here as the translation pass, when building the parse tree in a similar to that of any $LR(0)$ parser, will execute in $O(n)$ \emph{todo: O symbol??} time.
