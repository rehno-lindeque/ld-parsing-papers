...
[ \text{Text stream is an array of characters} ]
LexicalAnalysis(lexStream, textStream);
[ \text{lexStream is an array of lexical tokens output by the lexer ending with the special end-of-stream token \$.} ]
RecognitionPass(rulesStream, lexStream);
[ \text{rulesStream is an array of rule tokens output by the recognition pass, each of which corresponds to a specific grammar rule or the the special $ignore$ token.} ]
TranslationPass(parseTree, rulesStream, lexStream, textStream);
[ \text{parseTree is the final discrete syntax tree produced by applying each of the rules in the rulesStream. } ]
\CORP
