...
Over time many variants of LR parsing, such as LALR and SLR [citation: DeLemer, F. L. 1971. Simple LR(k) grammars. Commun. ACM 14, 453-460], have been developed as well as some optimizations [citation needed: Very Fast LR Parsing, Thomas J. Pennell].
...
\footnote{Typical lookahead mechanisms used in LR(k) or LALR(k) parser generators have $k = 1$ or $k = 2$.}.
...
Recursive ascent parsing, considered to be an analog to recursive descent for bottom-up parsing %\cite{13326, 47909, 770849}.
...
Recursive ascent has been advocated as a learning exercise for teaching LR parsing. %\cite{13326, 47909, 770849}.
...
However, as \cite{Parr95lland} suggests, a distinction can be made between \emph{language recognition} and \emph{language translation}.
...
Furthermore, many parsers allow the user to specify additional precedence rules because writing a correct $LR(k)$ parser may be hard.
...
We will neither praise nor challenge this wisdom in our paper although we do suggest that further research should be conducted in order to support such a claim \emph{todo: read up about this}.\\
...
LL parsing has proven amenable to extension to larger classes of grammars with the popular ANTLR parser presenting two variants: LL(*) [citation needed]\cite{} and more recently Predicated-LL \cite{} [citation needed].
...
Other previous work similar to ours include [\emph{TODO:these} \cite{1287949, Marc80, 991520, 146993}].
...
Next, the theoretical aspects of the parser is explored along with some formal definitions and proofs.
%Finally the implementation details of the parser driver and generator are shown in the last sections. 
%Our findings are then diseminated\emph{todo: correct word} in the \emph{Results} and \emph{Conclusion} sections along with the remaining short-comings and future work.
...
G_1 \equiv \quad & A_1 \rightarrow x\\
                 ...
                 & S_2 \rightarrow B b \$
\end{align*}}
\parbox{.7\textwidth}{This is a simple grammar requiring a fixed size lookahead in order to output one of the two production rules, $A_1$ or $B_1$.}
\end{tabular}
...
G_2 \equiv \quad & A_1 \rightarrow x\\
                 ...
                 & S_2 \rightarrow x b \$
\end{align*}}
\parbox{.7\textwidth}{In this grammar the rule $A_1$ may be output, or the possible reduction could be ignored if $b$ is in the lookahead.}
\end{tabular}
...
G_3 \equiv \quad & A_1 \rightarrow x y\\
                 ...
                 & S_2 \rightarrow C B \$
\end{align*}}
\parbox{.7\textwidth}{The rules $A_1$ and $C_1$, $B_1$ in this grammar are partially disjoint.}
\end{tabular}
...
G_4 \equiv \quad & A_1 \rightarrow x\\
                 ...
                 & S_2 \rightarrow B b \$
\end{align*}}
\parbox{.7\textwidth}{The rules $A_2$ and $B_2$ in this grammar are both in a form of \emph{tail recursion}. 
...
G_5 \equiv \quad & A_1 \rightarrow x\\
                 ...
                 & S_2 \rightarrow B b \$
\end{align*}}
\parbox{.7\textwidth}{The rules $A_2$ and $B_2$ are both partially disjoint and recursive. One is left recursive in this case while the other is right recursive and the combination
...
\end{align*}}
\parbox{.7\textwidth}{In this grammar the rules $A_1$ and $B_1$ are not recursive, however their lookahead includes a cycle in $C$. 
...
G_{10} \equiv \quad & A_1 \rightarrow x\\
                  ...
                  & S_2 \rightarrow B D \$
\end{align*}}
\parbox{.7\textwidth}{Sometimes the lookahead itself will be contained inside a cycle.
...
%    & \langle shift(x), delay(A, B), switch(y), switch(b), reduce(D_1), reduce(D_2), resolve(B), shift(\$), \\&\qquad reduce(S_2) \rangle,\\
 \{ & \langle shift(x), delay, switch(a), reduce(C_1), resolve(A), shift(\$), reduce(S_1) \rangle,\\
    ...
    & \langle shift(x), delay, switch(y), switch(a), reduce(C_1), reduce(C_2), resolve(A_1), shift(\$), \\&\qquad reduce(S_1) \rangle,\\
    & \langle shift(x), delay, switch(y), switch(b), reduce(D_1), reduce(D_2), resolve(B_1), shift(\$), \\&\qquad reduce(S_2) \rangle,\\
    & \textellipsis \}
\end{align*}}
...
G_{11} \equiv \quad & A_1 \rightarrow x\\
                  ...
                  & S_2 \rightarrow B b \$
\end{align*}}
\parbox{.7\textwidth}{One pathological case to take into account is a grammar where multiple nested recursion takes place in a single rule yet both branches of tree require lookahead outside of the cycles. In this case notice that $A$ and $B$ are both 
...
We could write these string as $(x(y*))+a$ and $(x(y*))+b$ where $+$ indicates one or more repetitions and $*$ zero or more repetitions using the Kleene closure \emph{[todo: citation and is Kleene spelt correctly?]}.}
\end{tabular}
...
\{ & \langle shift(x), reduce(A_1), switch(a), shift(\$), reduce(S_1) \rangle,\\
   ...
   & \langle shift(x), reduce(A_1), switch(y), reduce(C_1), reduce(A_2), switch(x), reduce(A_2), switch(a), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), reduce(B_1), switch(y), reduce(D_1), reduce(B_2), switch(x), reduce(B_2), switch(b), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), reduce(A_1), switch(y), reduce(C_1), reduce(A_2), switch(x), reduce(A_1), switch(y), reduce(C_1), reduce(A_2), switch(a), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), reduce(B_1), switch(y), reduce(D_1), reduce(B_2), switch(x), reduce(B_1), switch(y), reduce(D_1), reduce(B_2), switch(b), shift(\$), reduce(S_2) \rangle,\\
   & \textellipsis
   & \langle shift(x), reduce(A_1), switch(y), reduce(C_1), switch(y), reduce(A_2), switch(x), reduce(A_1), switch(a), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), reduce(B_1), switch(y), reduce(D_1), switch(y), reduce(B_2), switch(x), reduce(B_1), switch(b), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), reduce(A_1), switch(y), reduce(C_1), switch(y), reduce(A_2), switch(x), reduce(A_1), switch(y), reduce(C_1), reduce(A_2), switch(a), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), reduce(B_1), switch(y), reduce(D_1), switch(y), reduce(B_2), switch(x), reduce(B_1), switch(y), reduce(D_1), reduce(B_2), switch(b), shift(\$), reduce(S_2) \rangle,\\
   & \textellipsis \}
\end{align*}}
...
\{ & \langle shift(x), delay, switch(a), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(b), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, delay, switch(a), resolve(A_2), resolve(C_1), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, delay, switch(b), resolve(B_2), resolve(D_1), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, reduce(A_2), switch(x), delay, switch(a), resolve(A_2), resolve(C_1), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, delay, switch(x), delay, switch(y), switch(b), resolve(D_1), resolve(B_2), resolve(D_1), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, reduce(A_2), switch(x), delay, switch(y), delay, reduce(A_2), switch(a), resolve(C_1), resolve(A_1), resolve(C_1), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, reduce(B_2), switch(x), delay, switch(y), delay, reduce(B_2), switch(b), resolve(D_1), resolve(B_1), resolve(D_1), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \textellipsis
   & \langle shift(x), delay, switch(y), delay, switch(y), reduce(A_2), switch(x), reduce(A_1), switch(a), resolve(C_1), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, switch(y), reduce(B_2), switch(x), reduce(B_1), switch(b), resolve(D_1), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, switch(y), reduce(A_2), switch(x), reduce(A_1), switch(y), reduce(C_1), reduce(A_2), switch(a), resolve(C_1), resolve(A_1), shift(\$), reduce(S_1) \rangle,\\
   & \langle shift(x), delay, switch(y), delay, switch(y), reduce(B_2), switch(x), reduce(B_1), switch(y), reduce(D_1), reduce(B_2), switch(b), resolve(D_1), resolve(B_1), shift(\$), reduce(S_2) \rangle,\\
   & \textellipsis \}
\end{align*}}

This particular trace has special significance to our work as it is a counter-example for which it is not possible to generate this trace given our existing instruction set.
In the conclusion of this paper we will look at a possible approach to resolving this problem in future work.

Reasoning about the traces of these grammars becomes increasingly difficult as cycles are connected in different ways.
...
In other words, is there a function $E(G, n)$ producing a prefix-closed set of traces of the grammar $G$ with $n \in \mathbb{N}$ such that the following relations hold?
...
As discussed in the previous section, an LD parser recognizes grammar rules left-to-right, bottom-up manner. 
...
Then in the worst case the number of reduce instructions will be $(j - 1) \cdot n + 1$ since every token will be involved in at most $j$ productions, one of which has to be the root node of the parse tree.
...
In addition to this a prototype parser generator was implemented using our current work-in-progress construction algorithm.
...
It should be noted that it is possible to annotate delay actions with the possible reductions that could be performed at this step.
We see it as a possibility for future work that by generating new rule numbers for various combinations of these rules we could generate concrete syntax trees that include ambiguity.
...
It has been established that the LD instruction set is capable of handling a wide variety of unambiguous context-free grammars but will not accept the special case where lookahead is required outside multiple nested recursions.
We put forward that the instruction set could be extended, possibly by adding an alternative to the $return$ instruction that does not remove the last address from the callstack.

Knuth showed that determining whether a context-free grammar is ambiguous is undecidable in general\cite{knuth65}. 
This implies that certain concequences must be taken into account for our work. 
First off, determining whether a grammar is a suitable candidate for parsing is.
Although we have not yet experienced this problem we suspect that the issue may be manifested as infinite growth in the state graph during the construction algorithm if certain grammars are used.
If this effect does occur it can be simply circumvented by specifying a limiting depth for the generator to use. 
...
In particular we have omitted error recovery and semantic actions from our work thus far.
