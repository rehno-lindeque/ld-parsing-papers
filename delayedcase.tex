...
The $shift$ operation is used to step over input tokens deterministicly. 
...
In other words, given some rule token both the left-hand side and the right-hand side of the rule can be determined (although underlying data like identifier name and literals must be stored separately).
...
In these examples the value of certain state variable will not be represented directly and are considered to be implicit.
...
In order to recognize a string satisfying this grammar we will derive the following sequence of parsing steps:
\begin{enumerate}
...
Furthermore parsers using these instruction will exhibit exponential growth if no explicit $return$ operation is available.
...
\texttt{delay}\\
The delay operation adds a placeholder token called $ignore$ to the output rules which may be replaced later.
...
\texttt{resolve(rule)}\\
The resolve operation is similar to the reduce operation, but instead of immediately pushing a rule token onto the stack of output rules it instead replaces the placeholder token previously pushed by a delay action.
...
\begin{equation}
\infer{l, j, outputrules_{delays_m}, m \mapsto l-1, j+1, r, m-1}{actions_j = resolve(r)} \tag{Def 1.7}
\end{equation}\\

The pair of actions, `resolve' and `delay', works together in order to implement \emph{delayed reductions}. 
It is easy to see that the operation $delay$ immediately followed by the operation $resolve(r)$ is identical to our original operation $reduce(r)$ and could be used as a replacement\footnote{We've chosen to keep the original $reduce$ operation because, in practice, programming language grammars often require very few lookaheads.}.

\subsubsection{A modification to the `switch' operation}
In order for the nexed two operations to work correctly we must also modify our original $switch$ operation to record the state from which we've come.
...
A $return$ operation is needed in order to return to the state from which the previous $switch$ operation jumped. 
...
The variable $lasttarget$ is representative of all the relevant $switch$ jumps that that the parser needed to follow in order to recognize the needed lookahead up to this point.
...
\subsection{Traces}
To see how the delay/resolve pair can be used to look ahead in the input stream we must investigate several different grammar configurations.
...
This really implies that any recognition action directly followed by a $reduce$ operation may swap places. 
...
The state is expanded until a non-deterministic \emph{branch state} is reached where a parser would be required to make a choice corresponding to the $switch$ operation.
...
In the case of a $shift$ operation, the items in the original state is completed whereas, in the case of a $switch$, each of the new states are processed by the routine.\\
...
In the case of a $shift$ operation, the current state is tested and if all items are complete, a $return$ instruction must be generated. In 
\end{enumerate}
...
It is common practice to reduce these very basic recognition operations to constant time operations since the number of arguments to an instruction is fixed with respect to the number of input tokens.
...
We have already seen that the equivalent trace using delays may split a $reduce$ operation into a single pair of $delay$ and $resolve$ operation.
