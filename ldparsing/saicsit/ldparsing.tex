...
\begin{equation}
\infer{j, rulesbu\!f\!f\!er_k, k \mapsto j+1, r, k+1}{ops_j = reduce(r)} \tag{reduce}
\end{equation}

\begin{equation}
\infer{delays_m, m, j, rulesbu\!f\!f\!er_k, k \mapsto k, m+1, j+1, ignore, k+1}{ops_j = delay} \tag{delay}
\end{equation}

\begin{equation}
\infer{l, j, rulesbu\!f\!f\!er_{delays_{m-1}}, m \mapsto l-1, j+1, r, m-1}{ops_j = resolve(r)} \tag{resolve}
\end{equation}
...
{\small\begin{align*}
input & \quad \text{Array of lexical tokens}\\
i     & \quad \text{Index into the parser input stream} input_i\\
ops   & \quad \text{Sequence of parsing instructions (the parser}\\
      & \quad \text{definition)}\\
j     & \quad \text{Index into the parser instruction stream $ops_j$}\\
rulesbu\!f\!f\!er & \quad \text{Dynamically growing array of production rules}\\
k     & \quad \text{Index pointing to the first unassigned cell in the}\\
      & \quad \text{rules buffer $rulesbu\!f\!f\!er_k$}
\end{align*}}
